<!-- Basic Modal-->
<div class="modal fade" id="checkBukti{{$keuangan->id_keuangan}}" tabindex="-1">
                  ...
                  <div class="col-4">
                    <div class="row">
                        <div class="d-flex justify-content-between">
                            <form class="row g-3" action="{{ route('check.terima', $keuangan->id_keuangan) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="text" hidden value="Diterima" name="status">
                                <input type="text" hidden value="{{$keuangan->id_anggota}}" name="id_anggota">
                                <input type="number" hidden value="{{$keuangan->jumlah}}" name="jumlah">
                                <button type="submit" class="btn btn-success">Terima</button>
                            </form>
                            <form class="row g-3" action="{{ route('check.tolak', $keuangan->id_keuangan) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="text" hidden value="Ditolak" name="status">
